...
[← Документация](README.md) · Реализация в стиле SonarQube (on-demand, без постоянного хранения исходников на сервере)
...
На дашборде проекта → **Issues** → кнопка **Code** в строке предупреждения раскрывает inline-фрагмент (`partials/issue_row.html`, `toggleInlineCode` в `static/app.js`, `GET /ui/file`). Source roots настраиваются во вкладке **Settings → Source roots**.
...
Дашборд → **Settings** → **Source roots** (или API v2):
...
Дашборд → **Settings** → **Source roots**, или API:
...
- [quick-reference.md](quick-reference.md) — API upload, Git URL
- [jenkins-ci.md](jenkins-ci.md) — CI pipeline
- [CURSOR.md](../CURSOR.md) — `GET /ui/file`, `static/app.js`
