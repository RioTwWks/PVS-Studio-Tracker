# v0.2 — переход на SonarQube-like платформу

[← Документация](README.md)

> **Архив.** Часть документа устарела. Актуально: [CURSOR.md](../CURSOR.md), [.cursor/spec.md](../.cursor/spec.md), [quick-reference.md](quick-reference.md).  
> Quality gates — наборы **`rule_code`** (`QualityGateRule`), не только metric conditions. Также: профиль/email, `target_platform`, LDAP, автор issues, `.meta.json` upload.
